...
class BallManager{
    private:
        ...
        Paddle<FontTPad, ShapeT>* paddle_;
        ContT* conteneur_;

        bool running = true;

        void makeCollisions();
        inline float norme(sf::Vector2f vect) {return std::sqrt(vect.x * vect.x + vect.y * vect.y);};
    ...
    public:
        inline BallManager(Ball<FontTBall> &ball, ContT* conteneur, Paddle<FontTPad, ShapeT>* paddle): ball_{ball}, paddle_{paddle}, conteneur_{conteneur} {/*std::cout << "Ball manager :" << this << ", ball :" << ball_ << std::endl;*/};
